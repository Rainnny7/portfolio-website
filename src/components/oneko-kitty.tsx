...
const OnekoKitty = (): ReactElement => {
    const { showKitty: showKittyContext } = useKitty();
    const showKitty = !useIsScreenSize() && showKittyContext;
    ...
    function setSprite(name: SpriteName, frame: number) {
        ...
        if (!sprite) return;
        nekoElRef.current!.style.backgroundPosition = `${
            (sprite[0] ?? 0) * 32
        }px ${(sprite[1] ?? 0) * 32}px`;
    }
